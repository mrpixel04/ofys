<div>
    ...
    <form wire:submit.prevent="login" class="space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center">
                <input wire:model.live="remember" id="remember_me" type="checkbox"
        ...
        <div>
            <button type="submit" class="w-full flex justify-center py-3 px-4 border border-transparent rounded-2xl shadow-sm text-sm font-medium text-white bg-yellow-500 hover:bg-yellow-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-yellow-500 transition-colors duration-200">
                <span wire:loading.remove wire:target="login">{{ __('auth.login') }}</span>
    ...
    <div x-data="{ show: false }"
         x-init="$wire.on('loggedIn', () => { show = true; setTimeout(() => { show = false }, 3000) })"
         x-show="show"
            ...
            <div class="p-4">
                <div class="flex items-start">
                    ...
                    <div class="ml-3 w-0 flex-1 pt-0.5">
                        <p class="text-sm font-medium text-gray-900">{{ __('auth.login') }} {{ __('auth.success') }}!</p>
                        <p class="mt-1 text-sm text-gray-500">{{ __('auth.login_success') }}</p>
                    </div>
    ...
    <div x-data="{ show: false, message: '' }"
         x-init="$wire.on('loginError', (data) => { message = data.message; show = true; setTimeout(() => { show = false }, 3000) })"
         x-show="show"
         x-transition:enter="transform ease-out duration-300 transition"
         x-transition:enter-start="translate-y-2 opacity-0 sm:translate-y-0 sm:translate-x-2"
         x-transition:enter-end="translate-y-0 opacity-100 sm:translate-x-0"
        ...
        <div class="max-w-sm w-full bg-white shadow-lg rounded-lg pointer-events-auto overflow-hidden mx-auto">
            <div class="p-4">
                <div class="flex items-start">
                    ...
                    <div class="ml-3 w-0 flex-1 pt-0.5">
                        <p class="text-sm font-medium text-gray-900">{{ __('auth.login') }} {{ __('auth.failed') }}</p>
