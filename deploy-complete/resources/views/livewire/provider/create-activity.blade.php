<div class="bg-white rounded-lg shadow-lg overflow-hidden">
    ...
    <form wire:submit.prevent="saveActivity" class="p-6">
        ...
        <div class="space-y-8">
            ...
            <div>
                ...
                <div class="mt-4 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-2">
                    <div>
                        ...
                        <select id="activityType" wire:model.live="activityType"
                    ...
                    <div>
                        <label for="state" class="block text-base font-medium text-gray-700">State</label>
                        ...
                        <div class="mt-1 grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <div class="relative rounded-md shadow-sm">
                                    <input type="number" id="durationHours" wire:model="durationHours" min="0"
                                        class="focus:ring-teal-500 focus:border-teal-500 block w-full border-gray-300 rounded-md px-4 py-3 text-base">
                                </div>
                            </div>
                            <div>
            ...
            @if($showLotSection)
            <div>
                <h2 class="text-lg font-medium text-gray-900 border-b pb-2">Lot Locations</h2>
                <div class="mt-4">
                    <!-- Add New Lot Form -->
                    <div class="bg-gray-50 p-4 rounded-lg mb-4">
                        <h3 class="text-base font-medium text-gray-700 mb-3">Add New Lot</h3>
                        <div class="grid grid-cols-1 gap-y-4 sm:grid-cols-2 gap-x-4">
                            <div>
                                <label for="newLotDescription" class="block text-base font-medium text-gray-700">Description (Optional)</label>
                                <input type="text" id="newLotDescription" wire:model="newLotDescription"
                                    class="mt-1 focus:ring-teal-500 focus:border-teal-500 block w-full shadow-sm border-gray-300 rounded-md px-4 py-3 text-base"
                                    placeholder="e.g., Near the lake, Mountain view">
                            </div>
                            <div>
                                <label for="newLotCapacity" class="block text-base font-medium text-gray-700">Capacity</label>
                                <input type="number" id="newLotCapacity" wire:model="newLotCapacity" min="1"
                                    class="mt-1 focus:ring-teal-500 focus:border-teal-500 block w-full shadow-sm border-gray-300 rounded-md px-4 py-3 text-base">
                            </div>
                        </div>
                        <div class="mt-4">
                            <button type="button" wire:click="addLot"
                        ...
                        @forelse($lots as $index => $lot)
                            <div class="bg-white border border-gray-200 rounded-lg shadow-sm p-4">
                                <div class="flex items-center justify-between mb-3">
                                    ...
                                    <button type="button" wire:click="removeLot({{ $index }})"
                                ...
                                <div class="grid grid-cols-1 gap-y-4 sm:grid-cols-2 gap-x-4">
                                    <div>
                                        ...
                                        <input type="text" wire:model="lots.{{ $index }}.description"
                                            class="mt-1 focus:ring-teal-500 focus:border-teal-500 block w-full shadow-sm border-gray-300 rounded-md px-4 py-3 text-base">
                                    </div>
                                    <div>
                                        <label class="block text-base font-medium text-gray-700">Capacity</label>
                                        <input type="number" wire:model="lots.{{ $index }}.capacity" min="1"
                                            class="mt-1 focus:ring-teal-500 focus:border-teal-500 block w-full shadow-sm border-gray-300 rounded-md px-4 py-3 text-base">
                                    </div>
                                </div>
                                <div class="mt-4">
                                    <label class="inline-flex items-center">
                                        <input type="checkbox" wire:model="lots.{{ $index }}.is_available"
                                        ...
                                        <span class="ml-2 text-base text-gray-700">Available for booking</span>
                                    </label>
                                </div>
                            </div>
                        @empty
                            <div class="text-center py-8 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
                                ...
                                <h3 class="mt-2 text-base font-medium text-gray-900">No lots added yet</h3>
                                <p class="mt-1 text-base text-gray-500">Add your first lot location to get started.</p>
                            </div>
                        @endforelse
                    </div>
                </div>
            ...
            <!-- Images Section -->
            <div>
                <h2 class="text-lg font-medium text-gray-900 border-b pb-2">Activity Images</h2>
                <div class="mt-4">
                    <label class="block text-base font-medium text-gray-700 mb-2">Upload Images</label>
                    ...
                    <div class="mb-4">
                        ...
                        >
                            <label for="activityImages" class="flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md cursor-pointer hover:bg-gray-50">
                                <div class="space-y-1 text-center">
                                    <svg class="mx-auto h-12 w-12 text-gray-400" stroke="currentColor" fill="none" viewBox="0 0 48 48">
                                        <path d="M28 8H12a4 4 0 00-4 4v20m32-12v8m0 0v8a4 4 0 01-4 4H12a4 4 0 01-4-4v-4m32-4l-3.172-3.172a4 4 0 00-5.656 0L28 28M8 32l9.172-9.172a4 4 0 015.656 0L28 28m0 0l4 4m4-24h8m-4-4v8m-12 4h.02" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                    </svg>
                                    <div class="flex text-sm text-gray-600">
                                        <input id="activityImages" wire:model="activityImages" type="file" multiple class="sr-only">
                                        <p class="pl-1">Upload multiple images (5MB max per image)</p>
                                    </div>
                            ...
                            <div x-show="uploading" class="mt-2">
                                ...
                                <div class="text-center text-sm mt-1 text-gray-500">Uploading...</div>
                            </div>
                        </div>
                    </div>
                    ...
                    @if(count($activityImages) > 0)
                        <h3 class="font-medium text-gray-700 mb-2">New Images to Upload</h3>
                            ...
                            @foreach($activityImages as $index => $image)
                                <div class="relative group">
                                    <div class="aspect-w-16 aspect-h-9 rounded-lg overflow-hidden bg-gray-100">
                                        <img src="{{ $image->temporaryUrl() }}" alt="Activity image preview" class="w-full h-full object-cover">
                                    </div>
                                    <button type="button" wire:click="removeImage({{ $index }})" class="absolute top-2 right-2 p-1 bg-red-500 rounded-full text-white opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    @if($activityId && count($existingImages) > 0)
                        <h3 class="font-medium text-gray-700 mb-2">Existing Images</h3>
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                            @foreach($existingImages as $index => $image)
                                <div class="relative group">
                                    ...
                                    <button type="button" wire:click="removeExistingImage({{ $index }})" class="absolute top-2 right-2 p-1 bg-red-500 rounded-full text-white opacity-0 group-hover:opacity-100 transition-opacity">
                                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                                        </svg>
                                    </button>
