    ...
    <div class="container mx-auto px-4">
        <div class="max-w-5xl mx-auto">
            ...
            <div class="text-center mb-8">
                <h1 class="text-4xl font-extrabold text-gray-900 mb-3">{{ __('Status Pembayaran') }}</h1>
                <p class="text-xl text-gray-600">{{ __('Track your payment and booking details') }}</p>
            </div>
                ...
                <div class="bg-gradient-to-r {{ $booking->isPaid() ? 'from-green-500 to-emerald-600' : ($booking->isFailed() ? 'from-red-500 to-orange-600' : 'from-yellow-500 to-orange-500') }} px-8 py-6">
                    <div class="flex items-center justify-between text-white">
                        ...
                        <div class="text-right">
                            <p class="text-sm font-semibold uppercase tracking-wide opacity-90 mb-2">{{ __('Status Semasa') }}</p>
                            @if($booking->isPaid())
                        ...
                        <div class="space-y-8">
                            ...
                            <div class="relative flex items-start">
                                ...
                                <div class="ml-6 flex-1">
                                    <h3 class="text-lg font-bold text-gray-900">{{ __('Booking Created') }}</h3>
                                    <p class="text-sm text-gray-600">{{ $booking->created_at->format('M d, Y h:i A') }}</p>
                                    <p class="text-sm text-gray-500 mt-1">{{ __('Your booking was successfully created') }}</p>
                                </div>
                            </div>
                            ...
                            @if($booking->billplz_bill_id)
                                <div class="relative flex items-start">
                                    ...
                                    <div class="ml-6 flex-1">
                                        <h3 class="text-lg font-bold text-gray-900">{{ __('Payment Initiated') }}</h3>
                                        <p class="text-sm text-gray-600">{{ $booking->last_payment_attempt ? $booking->last_payment_attempt->format('M d, Y h:i A') : __('Recently') }}</p>
                                        <p class="text-sm text-gray-500 mt-1">{{ __('Bill ID') }}: <span class="font-mono font-semibold">{{ $booking->billplz_bill_id }}</span></p>
                                    </div>
                                </div>
                            @endif
                            ...
                            @if($booking->isPaid())
                                <div class="relative flex items-start">
                                    ...
                                    <div class="ml-6 flex-1">
                                        <h3 class="text-lg font-bold text-green-600">{{ __('Pembayaran Berjaya') }}</h3>
                                        <p class="text-sm text-gray-600">{{ $booking->billplz_paid_at ? $booking->billplz_paid_at->format('M d, Y h:i A') : __('Recently') }}</p>
                                        <p class="text-sm text-gray-500 mt-1">{{ __('Your payment has been confirmed') }}</p>
                                        @if($booking->billplz_transaction_id)
                                            <p class="text-sm text-gray-500">{{ __('Transaction ID') }}: <span class="font-mono font-semibold">{{ $booking->billplz_transaction_id }}</span></p>
                                        @endif
                                    </div>
                                </div>
                            @elseif($booking->isFailed())
                                <div class="relative flex items-start">
                                    ...
                                    <div class="ml-6 flex-1">
                                        <h3 class="text-lg font-bold text-red-600">{{ __('Pembayaran Gagal') }}</h3>
                                        <p class="text-sm text-gray-600">{{ $booking->last_payment_attempt ? $booking->last_payment_attempt->format('M d, Y h:i A') : __('Recently') }}</p>
                                        <p class="text-sm text-gray-500 mt-1">{{ __('Your payment could not be processed') }}</p>
                            ...
                            @else
                                <div class="relative flex items-start">
                                    ...
                                    <div class="ml-6 flex-1">
                                        <h3 class="text-lg font-bold text-yellow-600">{{ __('Pembayaran Tertunda') }}</h3>
                                        <p class="text-sm text-gray-600">{{ __('Awaiting payment confirmation') }}</p>
                                        <p class="text-sm text-gray-500 mt-1">{{ __('Please complete your payment to confirm the booking') }}</p>
                                    </div>
                                </div>
                            @endif
                        </div>
                ...
                <div class="p-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        <div class="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-5 border border-green-200">
                            <div class="flex items-center justify-between mb-2">
                                <p class="text-sm font-semibold text-gray-600 uppercase tracking-wide">{{ __('Jumlah Bayaran') }}</p>
                        ...
                        <div class="bg-gradient-to-br from-purple-50 to-indigo-50 rounded-xl p-5 border border-purple-200">
                            <div class="flex items-center justify-between mb-2">
                                <p class="text-sm font-semibold text-gray-600 uppercase tracking-wide">{{ __('Kaedah Pembayaran') }}</p>
                        ...
                        <div class="bg-gradient-to-br from-orange-50 to-yellow-50 rounded-xl p-5 border border-orange-200">
                            <div class="flex items-center justify-between mb-2">
                                <p class="text-sm font-semibold text-gray-600 uppercase tracking-wide">{{ __('Cubaan Pembayaran') }}</p>
                    ...
                    @if($booking->billplz_bill_id)
                        <div class="mt-6 bg-gray-50 rounded-xl p-6 border border-gray-200">
                            <h3 class="text-lg font-bold text-gray-900 mb-4">{{ __('Maklumat Transaksi') }}</h3>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <p class="text-xs text-gray-500 font-semibold uppercase tracking-wide mb-1">{{ __('ID Bil') }}</p>
                                    <p class="text-sm font-mono font-semibold text-gray-900">{{ $booking->billplz_bill_id }}</p>
                                </div>
                                @if($booking->billplz_transaction_id)
                                    <div>
                                        <p class="text-xs text-gray-500 font-semibold uppercase tracking-wide mb-1">{{ __('ID Transaksi') }}</p>
                                        <p class="text-sm font-mono font-semibold text-gray-900">{{ $booking->billplz_transaction_id }}</p>
                                    </div>
                                @endif
                                @if($booking->billplz_collection_id)
                                    <div>
                                        <p class="text-xs text-gray-500 font-semibold uppercase tracking-wide mb-1">{{ __('ID Kutipan') }}</p>
                                        <p class="text-sm font-mono font-semibold text-gray-900">{{ $booking->billplz_collection_id }}</p>
                                    </div>
                                @endif
                                @if($booking->last_payment_attempt)
                                    <div>
                                        <p class="text-xs text-gray-500 font-semibold uppercase tracking-wide mb-1">{{ __('Cubaan Terakhir') }}</p>
            ...
            <div class="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-200 mb-6">
                ...
                <div class="p-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <p class="text-sm text-gray-500 font-semibold mb-2">{{ __('Aktiviti') }}</p>
                            <p class="text-xl font-bold text-gray-900">{{ $booking->activity->name }}</p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-500 font-semibold mb-2">{{ __('Status Tempahan') }}</p>
                        ...
                        <div>
                            <p class="text-sm text-gray-500 font-semibold mb-2">{{ __('Tarikh & Masa') }}</p>
                        ...
                        <div>
                            <p class="text-sm text-gray-500 font-semibold mb-2">{{ __('Peserta') }}</p>
                        ...
                        @elseif($booking->isFailed() || $booking->isPending())
                            <form action="{{ route('payment.retry', $booking->id) }}" method="POST" class="inline-block">
                                ...
                                <button type="submit" class="w-full sm:w-auto inline-flex items-center justify-center px-8 py-3 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white font-bold rounded-xl transform hover:scale-105 transition-all duration-300 shadow-lg hover:shadow-xl">
                                    ...
                                    {{ $booking->isFailed() ? __('Cuba Semula Pembayaran') : __('Selesaikan Pembayaran') }}
                                </button>
                            </form>
                        @endif
            ...
            @if(!$booking->isPaid())
                <div class="mt-6 bg-blue-50 border-l-4 border-blue-500 rounded-xl p-6">
                    <div class="flex items-start">
                        ...
                        <div>
                            <h4 class="text-lg font-bold text-blue-900 mb-2">Perlukan Bantuan?</h4>
                            <p class="text-sm text-blue-800 mb-3">
                                Jika anda menghadapi isu pembayaran atau ada soalan, pasukan sokongan kami sedia membantu.
                            </p>
                            <p class="text-sm text-blue-800">
                                Hubungi kami di <strong>support@ofys.com</strong> atau telefon <strong>+60 12-345-6789</strong>
                            </p>
                        </div>
                    </div>
                </div>
