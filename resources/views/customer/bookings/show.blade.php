            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class="flex justify-between items-center mb-6">
                        <h1 class="text-2xl font-semibold text-gray-900">{{ __('Butiran Tempahan') }}</h1>
                        <div class="flex">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div class="md:col-span-2">
                            <div class="bg-gray-50 rounded-lg p-6 mb-6">
                                <h2 class="text-lg font-medium text-gray-900 mb-4">{{ __('Maklumat Aktiviti') }}</h2>
                                <dl class="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
                                    ...
                                    <div>
                                        <dt class="text-sm font-medium text-gray-500">{{ __('Jenis') }}</dt>
                                        <dd class="mt-1 text-sm text-gray-900">{{ $booking->activity_details['type'] ?? __('Tiada') }}</dd>
                                    </div>
                                </dl>
                            </div>

                            <div class="bg-gray-50 rounded-lg p-6 mb-6">
                                <h2 class="text-lg font-medium text-gray-900 mb-4">{{ __('Maklumat Tempahan') }}</h2>
                                <dl class="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
                                    <div>
                                        <dt class="text-sm font-medium text-gray-500">{{ __('Rujukan Tempahan') }}</dt>
                                        <dd class="mt-1 text-sm text-gray-900">{{ $booking->booking_reference }}</dd>
                                    </div>
                                    <div>
                                        <dt class="text-sm font-medium text-gray-500">{{ __('Tempahan Pada') }}</dt>
                            ...
                            <div class="bg-gray-50 rounded-lg p-6 mb-6">
                                <h2 class="text-lg font-medium text-gray-900 mb-4">{{ __('Butiran Tapak Perkhemahan') }}</h2>
                                <dl class="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
                                    <div>
                                        <dt class="text-sm font-medium text-gray-500">{{ __('ID Lot') }}</dt>
                                        <dd class="mt-1 text-sm text-gray-900">{{ $booking->activity_details['lot']['id'] }}</dd>
                                    </div>
                                    @if(!empty($booking->activity_details['lot']['name']))
                                    <div>
                                        <dt class="text-sm font-medium text-gray-500">{{ __('Nama Lot') }}</dt>
                                        <dd class="mt-1 text-sm text-gray-900">{{ $booking->activity_details['lot']['name'] }}</dd>
                                    </div>
                                    @endif
                                    @if(!empty($booking->activity_details['lot']['capacity']))
                                    <div>
                                        <dt class="text-sm font-medium text-gray-500">{{ __('Kapasiti') }}</dt>
                                        <dd class="mt-1 text-sm text-gray-900">{{ $booking->activity_details['lot']['capacity'] }} {{ $booking->activity_details['lot']['capacity'] == 1 ? __('orang') : __('orang') }}</dd>
                                    </div>
                                    @endif
                                    @if(!empty($booking->activity_details['lot']['description']))
                                    <div class="sm:col-span-2">
                                        <dt class="text-sm font-medium text-gray-500">{{ __('Penerangan') }}</dt>
                            ...
                            @if($booking->special_requests)
                                <div class="bg-gray-50 rounded-lg p-6 mb-6">
                                    <h2 class="text-lg font-medium text-gray-900 mb-4">{{ __('Permintaan Khas') }}</h2>
                            ...
                            @if($booking->status === 'cancelled' && $booking->cancelation_reason)
                                <div class="bg-red-50 rounded-lg p-6 mb-6">
                                    <h2 class="text-lg font-medium text-red-800 mb-4">{{ __('Maklumat Pembatalan') }}</h2>
                        ...
                        <div class="md:col-span-1">
                            <div class="bg-gray-50 rounded-lg p-6 sticky top-6">
                                <h2 class="text-lg font-medium text-gray-900 mb-4">{{ __('Ringkasan Pembayaran') }}</h2>

                                <div class="border-t border-gray-200 py-4">
                                    <dl class="space-y-2">
                                        <div class="flex justify-between">
                                            <dt class="text-sm text-gray-600">{{ __('Harga') }}</dt>
                                        ...
                                        @if($booking->activity_details['price_type'] === 'per_person')
                                            <div class="flex justify-between">
                                                <dt class="text-sm text-gray-600">{{ __('Peserta') }}</dt>
                                ...
                                <div class="border-t border-gray-200 pt-4">
                                    <div class="flex justify-between font-medium">
                                        <dt class="text-base text-gray-900">{{ __('Jumlah') }}</dt>
                                    ...
                                    <div class="mt-4">
                                        <div class="flex items-center">
                                            <dt class="text-sm text-gray-600 mr-2">{{ __('Status Pembayaran:') }}</dt>
                                            <dd>
                                    ...
                                    <div class="mt-6">
                                        <div class="bg-green-50 border-l-4 border-green-500 rounded-lg p-4 mb-4">
                                            <div class="flex items-center">
                                                ...
                                                <span class="text-sm font-semibold text-green-800">{{ __('Pembayaran Disahkan!') }}</span>
                                            </div>
                                        </div>
                                    ...
                                    <div class="mt-6">
                                        <div class="bg-red-50 border-l-4 border-red-500 rounded-lg p-4 mb-4">
                                            <div class="flex items-center">
                                                ...
                                                <span class="text-sm font-semibold text-red-800">{{ __('Pembayaran Gagal') }}</span>
                                            </div>
                                            <p class="text-xs text-red-700 mt-1">{{ __('Pembayaran anda tidak dapat diproses. Sila cuba lagi.') }}</p>
                                        </div>
                                    ...
                                    <div class="mt-6">
                                        <div class="bg-yellow-50 border-l-4 border-yellow-500 rounded-lg p-4 mb-4">
                                            <div class="flex items-center">
                                                ...
                                                <span class="text-sm font-semibold text-yellow-800">{{ __('Pembayaran Tertunda') }}</span>
                                            </div>
                                            <p class="text-xs text-yellow-700 mt-1">{{ __('Lengkapkan pembayaran untuk mengesahkan tempahan ini.') }}</p>
                                        </div>
                                        ...
                                        <p class="text-xs text-center text-gray-500 mt-2">
                                            {{ __('Pembayaran selamat melalui FPX, Kad Kredit, atau E-Wallet') }}
                                        </p>
                                        <a href="{{ route('payment.status', $booking->id) }}" class="mt-3 w-full inline-flex justify-center items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-yellow-500">
                                            <svg class="-ml-1 mr-2 h-5 w-5 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
                                            </svg>
                                            {{ __('Semak Status Pembayaran') }}
                                        </a>
                                </div>
                                @endif
                                ...
                                <div class="mt-6 space-y-3">
                                    ...
                                    @if($booking->status !== 'cancelled' && $booking->status !== 'completed')
                                        <form action="{{ route('customer.bookings.cancel', $booking->id) }}" method="POST" onsubmit="return confirm('Adakah anda pasti mahu membatalkan tempahan ini?');">
                                            @csrf
