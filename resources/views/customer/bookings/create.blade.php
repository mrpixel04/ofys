        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="mb-8">
                <a href="{{ route('activities.show', $activity->id) }}" class="flex items-center text-yellow-500 hover:text-yellow-600">
                    ...
                    {{ __('Kembali ke butiran aktiviti') }}
                </a>
            </div>

            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <h1 class="text-2xl font-semibold text-gray-900 mb-6">{{ __('Tempah :name', ['name' => $activity->name]) }}</h1>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div class="md:col-span-2">
                            <form action="{{ route('customer.bookings.store', $activity->id) }}" method="POST">
                                ...
                                <div class="bg-gray-50 rounded-lg p-6 mb-6">
                                    <h2 class="text-lg font-medium text-gray-900 mb-4">{{ __('Pilih Tarikh & Masa') }}</h2>

                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        <div>
                                            <label for="booking_date" class="block text-sm font-medium text-gray-700">{{ __('Tarikh') }}</label>
                                        ...
                                        <div>
                                            <label for="start_time" class="block text-sm font-medium text-gray-700">{{ __('Masa Mula') }}</label>
                                            <div class="mt-1 w-full">
                                                <select name="start_time" id="start_time" required
                                                    class="w-full h-10 px-4 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500 sm:text-sm">
                                                    <option value="">{{ __('Pilih masa') }}</option>
                                ...
                                <div class="bg-gray-50 rounded-lg p-6 mb-6">
                                    <h2 class="text-lg font-medium text-gray-900 mb-4">{{ __('Pilih Tapak Perkhemahan') }}</h2>
                                    ...
                                    @if($activity->lots->count() > 0)
                                                    ...
                                                    <div class="flex justify-between items-start">
                                                        <div class="flex items-start space-x-3">
                                                            ...
                                                            <div>
                                                                ...
                                                                @endif
                                                                <p class="text-sm text-gray-500">{{ __('Kapasiti: :count orang', ['count' => $lot->capacity]) }}</p>
                                                            </div>
                                                        </div>
                                                        <span class="inline-flex items-center {{ $lot->is_available ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }} px-2.5 py-0.5 rounded-full text-xs font-medium">
                                                            {{ $lot->is_available ? __('Tersedia') : __('Ditempah') }}
                                                        </span>
                                                    </div>
                                                </div>
                                            @endforeach
                                        ...
                                        @enderror
                                    @else
                                        <p class="text-sm text-gray-500 italic">{{ __('Tiada tapak perkhemahan tersedia untuk aktiviti ini. Sila hubungi penyedia.') }}</p>
                                    @endif
                                </div>
                                @endif
                                ...
                                <div class="bg-gray-50 rounded-lg p-6 mb-6">
                                    <h2 class="text-lg font-medium text-gray-900 mb-4">{{ __('Peserta') }}</h2>

                                    <div>
                                        <label for="participants" class="block text-sm font-medium text-gray-700">{{ __('Bilangan Peserta') }}</label>
                                        <div class="mt-1 w-full">
                                            <select name="participants" id="participants" required
                                                class="w-full h-10 px-4 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500 sm:text-sm">
                                                @for($i = $activity->min_participants; $i <= $activity->max_participants; $i++)
                                                    <option value="{{ $i }}">{{ $i }} {{ $i === 1 ? __('orang') : __('orang') }}</option>
                                                @endfor
                                            </select>
                                        </div>
                                        @error('participants')
                                ...
                                <div class="bg-gray-50 rounded-lg p-6 mb-6">
                                    ...
                                    <div>
                                        <label for="special_requests" class="block text-sm font-medium text-gray-700">{{ __('Ada permintaan atau keperluan khas?') }}</label>
                                        <div class="mt-1 w-full">
                                            <textarea name="special_requests" id="special_requests" rows="3"
                                                ...
                                                placeholder="{{ __('Contoh: alahan makanan, keperluan akses, dan lain-lain.') }}"></textarea>
                                        </div>
                        ...
                        <div class="md:col-span-1">
                            <div class="bg-gray-50 rounded-lg p-6 sticky top-6">
                                <h2 class="text-lg font-medium text-gray-900 mb-4">{{ __('Ringkasan Tempahan') }}</h2>
                                ...
                                <div class="mb-4">
                                    <div class="aspect-w-16 aspect-h-9 mb-4">
                                        ...
                                        @else
                                            <img src="https://via.placeholder.com/300x200?text=Tiada+Imej"
                                                alt="{{ __('Tiada imej tersedia') }}"
                                                class="object-cover rounded-lg">
                                        @endif
                                    </div>
                                ...
                                <div class="border-t border-gray-200 py-4">
                                    <dl class="space-y-2">
                                        ...
                                        <div class="flex justify-between">
                                            <dt class="text-sm text-gray-600">{{ __('Tempoh') }}</dt>
                                            <dd class="text-sm font-medium text-gray-900">
                                                {{ floor($activity->duration_minutes / 60) }} {{ __('jam') }} {{ $activity->duration_minutes % 60 }} {{ __('minit') }}
                                            </dd>
                                        </div>

                                        @if($activity->includes_gear)
                                            <div class="flex justify-between">
                                                <dt class="text-sm text-gray-600">{{ __('Peralatan') }}</dt>
                                                <dd class="text-sm font-medium text-gray-900">{{ __('Termasuk') }}</dd>
                                            </div>
                                        @endif
                                    </dl>
                                </div>
                                ...
                                <div class="border-t border-gray-200 pt-4">
                                    ...
                                    <p class="text-xs text-gray-500 mt-1">
                                        {{ __('Harga akhir akan dikira berdasarkan bilangan peserta') }}
                                    </p>
                                </div>
                            </div>
                        </div>
